...
denial checks run their own probe in a shared module sandbox; the X32 reject,
setuid/setcap helpers, and the same-pgid subtree take dedicated sandboxes. Each
test is named after its SS-ID and carries the catalog's Guards text as the assert
message.
...
def test_ss_h04_x32_abi_rejected(module_sandbox, probe):
    ...
    if _TARGET_IS_X86_64:
        ...
        report = parse_probe_output(result.get("output", ""))
        assert result.get("status") == "ok" and report.get("x32_unavailable") == "aarch64", (
            f"x32 ABI must be absent on the native aarch64 target | {result}"
        )
...
def test_ss_h12_file_caps_defeated(fresh_sandbox):
    staged = exec_cmd(
        ...
        "./eos_shell_security_probe setfilecap /tmp/eos-cap-helper | grep -q setfilecap=OK && "
        "echo SETCAP_OK && "
