...
def _phase_analysis(case_artifacts, phase: dict, name: str, arm: str, repetition: int) -> dict:
...
def test_manager_resource_traffic_is_quiescent(
    ...
    for repetition in range(1, repetitions + 1):
        ...
        pre = {
            "target": sample(case_artifacts, target, phase="resource-pre", repetition=repetition),
            "control": sample(case_artifacts, control, phase="resource-pre", repetition=repetition),
        }
        stores_before = {"target": fingerprint_store(target), "control": fingerprint_store(control)}
        ...
        post = {
            "target": sample(case_artifacts, target, phase="resource-post", repetition=repetition),
            "control": sample(case_artifacts, control, phase="resource-post", repetition=repetition),
        }
        stores_after = {"target": fingerprint_store(target), "control": fingerprint_store(control)}
        analyses = {
            arm: _phase_analysis(case_artifacts, measured, "resource-traffic", arm, repetition)
            for arm in ("target", "control")
        }
        deltas = {arm: resource_delta(pre[arm], post[arm]) for arm in ("target", "control")}
        result = {
            "repetition": repetition,
            "warm": warm,
            "campaign": campaign,
            ...
            "target_minus_control": {
                "cpu_ticks_per_minute": analyses["target"]["cpu_ticks_per_minute"] - analyses["control"]["cpu_ticks_per_minute"],
                "anonymous_growth_bytes": deltas["target"]["anonymous_bytes"] - deltas["control"]["anonymous_bytes"],
            },
            "stores_before": stores_before,
            "stores_after": stores_after,
        }
        results.append(result)
        case_artifacts.append_jsonl("route-traffic.json", result)
    ...
    cooldown = stream_group(
        ...
        repetition=1,
        duration_seconds=strict_duration("E2E_RE04_COOLDOWN_SECONDS", 600, minimum=600),
        interval_seconds=5,
    )
    cooldown_target = _phase_analysis(case_artifacts, cooldown, "resource-cooldown", "target", 1)
    ring = host_file_stat(ring_path)
    summary = {"repetitions": results, "unreachable": unreachable, "cooldown": cooldown_target, "ring": ring}
    case_artifacts.write_json("summary.json", summary, reserved=True)

    with validation(
        "resource-series-available",
        expected={"successful_reads": reads * repetitions, "paused_daemon_available": True},
        actual={"campaigns": [item["campaign"] for item in results], "paused": unreachable.get("availability")},
        evidence=("route-traffic.json", "summary.json"),
    ):
        assert all(item["campaign"]["success_count"] == reads and item["campaign"]["error_count"] == 0 for item in results)
        assert unreachable["availability"] in {"available", "partial"}

    with validation(
        "daemon-quiescent",
        expected={"target_minus_control_ticks_per_minute_lt": 1, "target_io_delta": 0, "anonymous_growth_max": ROUTE_MEMORY_DELTA_BYTES},
        actual=[{"difference": item["target_minus_control"], "deltas": item["deltas"]} for item in results],
        evidence=("samples.jsonl", "route-traffic.json"),
    ):
        for item in results:
            assert item["target_minus_control"]["cpu_ticks_per_minute"] < CPU_TICK_BUDGET_PER_MINUTE
            assert item["deltas"]["target"]["read_bytes"] == 0
            assert item["deltas"]["target"]["write_bytes"] == 0
            assert item["target_minus_control"]["anonymous_growth_bytes"] <= ROUTE_MEMORY_DELTA_BYTES

    with validation(
        "store-read-pure",
        expected="all target/control fingerprints unchanged",
        actual=[{"before": item["stores_before"], "after": item["stores_after"]} for item in results],
        evidence=("route-traffic.json",),
    ):
        for item in results:
            for arm in ("target", "control"):
                assert_store_unchanged(item["stores_before"][arm], item["stores_after"][arm])

    with validation(
        "post-poll-cooldown",
        expected={"anonymous_delta_max": COOLDOWN_ANONYMOUS_DELTA_BYTES, "ring_max_bytes": MAX_RING_BYTES},
        actual={"cooldown": cooldown_target, "ring": ring},
        evidence=("samples.jsonl", "summary.json"),
    ):
        pre_median = statistics.median(item["analysis"]["target"]["first_window_median_bytes"] for item in results)
        assert cooldown_target["final_window_median_bytes"] - pre_median <= COOLDOWN_ANONYMOUS_DELTA_BYTES
        assert ring.get("exists") is True and ring["logical_bytes"] <= MAX_RING_BYTES
...
def test_explicit_topology_cost(
    ...
    baseline_store = fingerprint_store(sandbox_id)
    ...
    noop_analysis = _phase_analysis(case_artifacts, noop_phase, "topology-noop", "target", 1)
    ...
    def exercise(name: str, repetition: int, verify):
        ...
        return phase, traffic, _phase_analysis(case_artifacts, phase, name, "target", repetition)

    def verify_empty(response):
        assert response["topology"]["workspaces"] == [], response

    empty_phase, empty_traffic, empty_analysis = exercise("topology-empty", 1, verify_empty)
    workspace_id = create_workspace(tracker)
    idle_phase, idle_traffic, idle_analysis = exercise(
        "topology-idle", 1, lambda response: workspace_by_id(response["topology"], workspace_id)
    )
    command_id = start_command(tracker, workspace_id, "sleep 590", timeout_ms=700_000)
    measured_workspace, measured_process, measured_identity = measure_namespace_identity(
        sandbox_id,
        workspace_id,
    )
    ...
    active_seen = {"value": False, "pids": set()}

    def verify_active(response):
        workspace = workspace_by_id(response["topology"], workspace_id)
        rows = workload_processes(workspace)
        if rows:
            active_seen["value"] = True
            active_seen["pids"].update(row["pid"] for row in rows)

    active_phase, active_traffic, active_analysis = exercise("topology-active", 1, verify_active)
    terminal = await_command(tracker, command_id, timeout_seconds=120)
    assert terminal.get("status") == "ok", terminal

    def topology_without_command():
        value = read_topology(sandbox_id)
        return value if not workload_processes(workspace_by_id(value, workspace_id)) else None
    ...
    cooldown_analysis = _phase_analysis(case_artifacts, cooldown, "topology-cooldown", "target", 1)
    final_sample = sample(case_artifacts, sandbox_id, phase="topology-final")
    final_store = fingerprint_store(sandbox_id)
    summary = {
    ...
    with validation(
        ...
        actual={"empty": empty_analysis, "noop": noop_analysis, "traffic": empty_traffic},
        evidence=("samples.jsonl", "route-traffic.json"),
    ):
        ...
        assert empty_analysis["cpu_ticks_per_minute"] - noop_analysis["cpu_ticks_per_minute"] <= CPU_TICK_BUDGET_PER_MINUTE

    with validation(
        "idle-topology-bounded",
        expected={"responses": requests, "cpu_fraction_max": 0.005},
        actual={"idle": idle_analysis, "traffic": idle_traffic},
        evidence=("samples.jsonl", "route-traffic.json"),
    ):
        # CLK_TCK is normally 100; 0.5% of one core is 0.5 ticks/second.
        assert idle_traffic["success_count"] == requests
        assert idle_analysis["cpu_ticks_per_minute"] <= 30
    ...
    with validation(
        ...
        assert active_seen["pids"].isdisjoint(
            row["pid"] for row in workload_processes(workspace_by_id(gone_topology, workspace_id))
        )

    with validation(
        "topology-cooldown",
        expected={"store_unchanged": True, "slope_max": ANONYMOUS_SLOPE_BYTES_PER_HOUR},
        actual={"cooldown": cooldown_analysis, "threads": final_sample["process"]["threads"]},
        evidence=("samples.jsonl", "summary.json"),
    ):
        assert_store_unchanged(baseline_store, final_store)
        assert cooldown_analysis["anonymous_slope_bytes_per_hour"] <= ANONYMOUS_SLOPE_BYTES_PER_HOUR
        assert final_sample["process"]["threads"] == baseline_sample["process"]["threads"]
        assert final_sample["smaps"]["Anonymous"] - baseline_sample["smaps"]["Anonymous"] <= COOLDOWN_ANONYMOUS_DELTA_BYTES
...
def test_fleet_resource_scaling(
    ...
    rings = {sandbox_id: default_resource_ring_path(sandbox_id) for sandbox_id in sandboxes}
    warm = stream_group(
        case_artifacts,
        [(sandbox_id, f"sandbox-{index:02d}", rings[sandbox_id]) for index, sandbox_id in enumerate(sandboxes)],
        phase="fleet-warm",
        repetition=1,
    ...
    stores_before = {sandbox_id: fingerprint_store(sandbox_id) for sandbox_id in sandboxes}
    ...
    def verify(response):
        ...
        assert observed == expected, {"missing": sorted(expected - observed), "unrelated": sorted(observed - expected)}
        index = round_robin["index"] % len(sandboxes)
        sample(case_artifacts, sandboxes[index], phase="fleet-traffic", repetition=index + 1)
        round_robin["index"] += 1
    ...
    if manager_points[-1][0] != float(requests):
        manager_points.append((float(requests), float(manager_after["anonymous_bytes"])))
    after = {
        sandbox_id: sample(case_artifacts, sandbox_id, phase="fleet-after")
        for sandbox_id in sandboxes
    }
    stores_after = {sandbox_id: fingerprint_store(sandbox_id) for sandbox_id in sandboxes}
    daemon_deltas = {sandbox_id: resource_delta(before[sandbox_id], after[sandbox_id]) for sandbox_id in sandboxes}
    ring_stats = {sandbox_id: host_file_stat(path) for sandbox_id, path in rings.items()}
    manager_delta = {
        key: manager_after[key] - manager_before[key]
        for key in ("anonymous_bytes", "rss_bytes", "user_ticks", "system_ticks", "read_bytes", "write_bytes")
    }
    assert (
        manager_after["pid"],
    ...
    case_artifacts.write_json("route-traffic.json", campaign)
    ...
    with validation(
        ...
        for sandbox_id, delta in daemon_deltas.items():
            assert (delta["user_ticks"] + delta["system_ticks"]) / elapsed_minutes < CPU_TICK_BUDGET_PER_MINUTE, {sandbox_id: delta}
            assert delta["read_bytes"] == 0 and delta["write_bytes"] == 0, {sandbox_id: delta}
            assert delta["anonymous_bytes"] <= ROUTE_MEMORY_DELTA_BYTES, {sandbox_id: delta}
            difference = daemon_minus_control[sandbox_id]
            assert abs(
                difference["user_ticks"] + difference["system_ticks"]
            ) / elapsed_minutes < CPU_TICK_BUDGET_PER_MINUTE, {
    ...
    with validation(
        ...
        assert all(value.get("exists") is True and value["logical_bytes"] <= MAX_RING_BYTES for value in ring_stats.values())
    ...
    with validation(
        ...
        actual={"destroyed": len(registered_sandbox_factory.destroyed), "rings": cleanup},
