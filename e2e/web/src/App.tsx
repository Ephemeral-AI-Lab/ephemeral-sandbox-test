...
function RunPage() {
  ...
  return <Stack className="page run-page" gap="xl">
    ...
    })}</Stack> : <FactGap>No case projections were published.</FactGap>}</section><RunCaseDetail item={selectedCase} failures={data.failures ?? []} evidencePurged={evidencePurged} onEvidence={(record) => selectedCase && setSelectedEvidence({ caseIdentity: `${selectedCase.test_id}:${selectedCase.case_id}`, record })} /></div>
...
function RunCaseDetail({ item, failures, evidencePurged, onEvidence }: { item: NonNullable<RunProjection["cases"]>[number] | undefined; failures: NonNullable<RunProjection["failures"]>; evidencePurged: boolean; onEvidence: (record: EvidenceRecord) => void }) {
