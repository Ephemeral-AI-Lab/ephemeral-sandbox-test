(function (root) {
  ...
  const state = { query: '', category: 'All', sort: 'featured', page: 1, selected: {}, cart: [], promo: '', wishlist: new Set(JSON.parse(localStorage.getItem('flashcart-wishlist') || '[]')), order: null };
  ...
  function persistWishlist() { localStorage.setItem('flashcart-wishlist', JSON.stringify([...state.wishlist].sort())); }
  ...
  function productCard(product) {
    ...
    card.append(button(state.wishlist.has(product.id) ? 'Saved' : 'Save to wishlist', () => { state.wishlist.has(product.id) ? state.wishlist.delete(product.id) : state.wishlist.add(product.id); persistWishlist(); render(); }, { class: 'quiet', 'aria-pressed': String(state.wishlist.has(product.id)) }));
